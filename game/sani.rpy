...
label sani:
    ...
    nk "Ah, esto se pone cada vez mejor."
    ...
    ya "¡Por que se lo que se siente!"
    ...
    "Yanami deja las llaves de una bicicleta en la mesa y se las entrega a Sosuke, con una mirada triste y melancolica en su rostro."

    ya "Solo ve. Karen te esta esperando."

    hs "¿Estas segura?"

    ya "Ella es paciente, pero no la dejes esperando demaciado."

    scene cgcafeteria3
    with Dissolve(.1)

    hs "Gracias Anna. Prometo no arruinarlo esta vez."

    "Sosuke agarra las llaves y se levanta, pero se detiene un momento, como si fuera a decir algo."

    scene cgcafeteria4
    with Dissolve(.1)

    "Pero tras un segundo, se va, sin mirar atras."
    ...
    "Mientras tanto, aqui estoy yo, viendo todo este drama desde mi propio asiento."

    "Creo que lo mejor sera pretender que no vi nada y seguir{cps=2}...{/cps} {p}{i}No... {w=.5}de verdad no esta...{/i}"

    scene cgcafeteria5
    with Dissolve(.1)

    "Ahi esta, Anna Yanami, a punto de tomar del vaso de la persona que le destroso el corazon hace unos instantes."

    "Cometiendo el acto más desesperado que se pueda cometer en este tipo de situaciones."

    scene cgcafeteria6
    with Dissolve(.1)

    "{i}{b}¡No lo hagas! ¡Te lo suplico, no lo hagas!{/b}{p}Pero ya es demasiado tarde.{/i}"

    "Toma el vaso con sus dos manos,"
    ...
    extend "{w=.1} y lentamente pone el pitillo en su boca, tomando de el."

    scene cafeteria
    with Dissolve(.5)

    "{i}{b}¡Ya esta! ¡Eso es cruzar una linea!{/i}{/b}"

    "Creo que eso fue sufi{nw}"
